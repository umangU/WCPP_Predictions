...
Y <- window(subset_data_ts, start=c(2004, 1), end=c(2004, 30))
y <- log10(Y)
...
ll_pred <- predict(ll_fit, horizon = 5)
...
llt_pred <- predict(llt_fit, horizon = 5)
...
lts_ss <- AddSeasonal(lts_ss, y, nseasons = 30)
lts_fit <- bsts(y, state.specification = lts_ss, niter = 2000)
lts_pred <- predict(lts_fit)
...
Actual <- data.frame(sample[31:60,2])
Predicted <- data.frame(d2[1:30,])
